...
// grain of the terrain", the confabulation guard. This module reads a task's
// place on that cube, reusing core/cube.js as the authority rather than minting a
// second vocabulary.
//
// THE THREE TASK ACTS ARE CUBE OPERATORS (the Act face, core/operators.js):
//
...
// Kafka confab the cube rejects. For tasks that is exactly: a Figure-maker handed
// a goal that is really Pattern/Ground-grained — a goal too big for one reach,
// jammed into a single generation because a guard capped the decomposition or the
// planner stopped splitting too early. `grainCoherence` flags it; the decomposer
// reads it as a STOPPING RULE — keep splitting while a goal is Pattern-grained,
// make a leaf only once it is Figure-grained.

import { cellOf, coherence, GRAINS } from '../core/index.js';

// The cube operator each task act IS. Imported here as ids; core/cube fixes their
