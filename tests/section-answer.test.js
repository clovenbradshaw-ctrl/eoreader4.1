...
import { sectionText, renderSectioned } from '../src/reader/section-answer.js';
...
test('a short answer is not sectioned — one idea, a tight paragraph', async () => {
  const r = await sectionText('Charles Lindbergh made the first solo nonstop transatlantic flight in 1927.');
  assert.equal(r.sectioned, false);
  assert.equal(r.sections.length, 1);
  assert.equal(r.sections[0].heading, null);
});

test('a multi-topic answer is sectioned, and the first segment stays heading-less (the lead)', async () => {
  const r = await sectionText(LONG, { leads: ['periscope', 'compass', 'instruments', 'windows'] });
...
test('headings are earned — drawn from the discourse leads when they land in a segment', async () => {
  const r = await sectionText(LONG, { leads: ['periscope', 'compass'] });
...
test('renderSectioned emits ## before labelled segments only', async () => {
  const r = await sectionText(LONG, { leads: ['periscope', 'compass'] });
